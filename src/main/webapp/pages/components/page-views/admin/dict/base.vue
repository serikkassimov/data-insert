...
<template id="page-views-admin-dict-base">
	<div>
		<h3>Справочник: {{dict.name}}</h3>
		<div class="grid-content" v-loading.body="items.loading">
...
(function($) {
	...
	var ajaxRoot = WorldClassRestRoot + '/dict/base';
	Vue.component(componentName, {
		...
		methods: {
			reloadItems: function() {
				...
				$.ajax({
					url: ajaxRoot + '/' + this.dict.type + '/list',
					dataType: 'json',
					context: this,
			...
			saveItemValidationCallback: function(valid) {
				...
				if (valid) {
					...
					$.ajax({
						url: ajaxRoot + '/' + this.dict.type + '/save',
						method: 'POST',
						data: data,
			...
			enableItemCallback: function(id) {
				...
				this.items.loading = true;
				$.ajax({
					url: ajaxRoot + '/' + this.dict.type + '/enable?id=' + id,
					dataType: 'json',
					context: this,
					error: function(jqXHR, textStatus, errorThrown) {
						this.$notify.error({
			...
			disableItemCallback: function(id) {
				...
				this.items.loading = true;
				$.ajax({
					url: ajaxRoot + '/' + this.dict.type + '/disable?id=' + id,
					dataType: 'json',
					context: this,
					error: function(jqXHR, textStatus, errorThrown) {
						this.$notify.error({
